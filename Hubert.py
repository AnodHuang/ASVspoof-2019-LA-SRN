# train_hubert_stream_4060_fix.py
import os
import json
import time
import math
...
def parse_args():
    ...
    p.add_argument("--data_dir", type=str, default=r"D:\capstone\ASV_Spoof_2019_LA_SNR_50MB")
    p.add_argument("--model_dir", type=str, default=r"D:\capstone\models\hubert-base-ls960")
    p.add_argument("--out", type=str, default=r"D:\capstone\hubert_asvspoof_stream_out_4060")
    ...
    p.add_argument("--train_buffer_shuffle", type=int, default=8000)
    p.add_argument("--val_take", type=int, default=0)

    p.add_argument("--fp16", action="store_true", default=True)

    # ✅ Windows + streaming：强烈建议 0，先跑通再说
...
class StreamCollator:
    def __init__(self, feature_extractor, member2label, sr=16000, max_sec=5.0):
...
def main():
    ...
    # ✅ 关键修复：关闭 datasets 的 Audio 自动解码（避免 torchcodec/ffmpeg）
    train_stream = load_dataset("parquet", data_files={"train": train_files}, streaming=True)["train"].decode(False)
    train_stream = train_stream.shuffle(buffer_size=args.train_buffer_shuffle, seed=42)

    val_stream = load_dataset("parquet", data_files={"validation": val_files}, streaming=True)["validation"].decode(False)
