...
JSONL_KEY_COL = "member"   # jsonl 里用于关联 parquet.__key__
JSONL_LABEL_COL = "key"    # jsonl 里的标签字段（0/1 或 bonafide/spoof）
...
def parse_args():
    ...
    p.add_argument("--model_dir", type=str, default="./mattyb95")
    p.add_argument("--out", type=str, default="./matty_asvspoof_stream_out")
    ...
    p.add_argument("--log_every", type=int, default=20)   # 每多少 step 打印一次
    p.add_argument("--eval_every_epoch", action="store_true", default=True)

    p.add_argument("--train_buffer_shuffle", type=int, default=20000)  # streaming shuffle buffer
    p.add_argument("--val_take", type=int, default=0)  # 0=全量验证；>0 只取前N条做快速验证
...
def find_jsonl(data_dir: str, split: str):
    # 你现在的路径是 data_dir/index/{split}.jsonl
    cands = [
...
def cheap_resample(x: np.ndarray, sr0: int, sr1: int):
    if sr0 == sr1:
        return x
    # 够用的线性插值（训练建议你数据本来就是 16k，这里只是保险）
...
def eval_loop(model, dl, device):
    ...
    for batch in dl:
        ...
        probs = F.softmax(logits, dim=-1)[:, 1]  # bonafide prob
    ...
    # metrics: acc, f1, roc_auc (手写，避免 evaluate 的额外依赖/缓存)
...
def main():
    ...
    train_dl = DataLoader(train_stream, batch_size=args.batch, num_workers=0, collate_fn=train_collator)
    val_dl = DataLoader(val_stream, batch_size=args.batch, num_workers=0, collate_fn=val_collator)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 训练：streaming 没有 epoch end 的概念 -> 你需要“每个 epoch 跑固定 steps”
        # 这里给一个合理默认：按你之前看到的 train=45600，steps ≈ 45600 / batch
        steps_per_epoch = max(1, math.ceil(45600 / args.batch))
